import { BackendError, type FragranceRequestRow, type FragranceRequestService, type FragranceRequestVoteCountRow, type FragranceRequestVoteRow, parseSchema, unwrapOrThrow } from '@aromi/shared'
...
export class VoteOnFRResolver extends AuthenticatedRequestResolver<MutationResolvers['voteOnFragranceRequest']> {
  ...
  private async getRequest (): Promise<FragranceRequestRow> {
  ...
  private async getExistingVote (): Promise<FragranceRequestVoteRow | null> {
    const { me, args } = this
  ...
  private async insertVote (): Promise<FragranceRequestVoteRow> {
    const { me, args } = this
  ...
  private async updateVote (
    oldVote: number
  ): Promise<FragranceRequestVoteRow> {
    const { me, args } = this
  ...
  private async getVoteCount (): Promise<FragranceRequestVoteCountRow> {
    const { requestId } = this.args.input
