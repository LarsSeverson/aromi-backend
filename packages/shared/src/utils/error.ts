...
export class ApiError extends GraphQLError {
  ...
  static fromSharp (error: unknown): ApiError {
    const typed = error as Error
    return new ApiError(
  ...
  static fromVibrant (error: unknown): ApiError {
    const typed = error as Error
    return new ApiError(
...
export const MEILI_ERROR_TO_API_ERROR: Record<string, ApiError> = {
  MeiliSearchCommunicationError: new ApiError('SEARCH_COMMUNICATION_ERROR', 'Could not reach search Service.js', 503),
...
export const MQ_ERROR_TO_API_ERROR: Record<string, ApiError> = {
  ...
  ConnectionError: new ApiError('QUEUE_CONNECTION_ERROR', 'Could not connect to queue Service.js', 503),
