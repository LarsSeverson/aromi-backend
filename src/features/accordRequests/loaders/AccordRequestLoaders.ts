...
import { type AccordRequestImageRow, type AccordRequestLoadersKey } from '../types'
      ...
      return await votes
        .findVoteInfos(
          eb => {
            if (myId != null) {
              return eb.and([
                eb('accordRequestVotes.requestId', 'in', keys),
                eb('accordRequestVotes.userId', '=', myId)
              ])
            }

            return eb('accordRequestVotes.requestId', 'in', keys)
          }
        )
        .match(
          rows => {
